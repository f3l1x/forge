...
class Thumbator extends Object
{
    ...
    /** @var array */
    private $maskHolders = ['%width%', '%height%', '%filename%', '%ext%', '%method%'];
    ...
    protected function mask($file, $width, $height, $method)
    {
        ...
        $replacements = [$normalize($width), $normalize($height), $pathinfo['filename'], $pathinfo['extension'], $this->method2name($method)];
